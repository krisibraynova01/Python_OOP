...
class Document:
    ...
    @classmethod
    def from_instances(cls, id: int, category: Category, topic: Topic, file_name):
    ...
    def __repr__(self):
        return f"Document {self.id}: {self.file_name}; category {self.category_id}, topic {self.topic_id}, tags: {', '.join(t for t in self.tags)}"
